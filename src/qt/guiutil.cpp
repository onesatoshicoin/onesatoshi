...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("onesatoshicoin"))
        return false;
...
{
    // Convert onesatoshicoin:// to onesatoshicoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("onesatoshicoin://"))
    {
        uri.replace(0, 12, "onesatoshicoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OneSatoshiCoin.lnk";
}
...
{
    return GetAutostartDir() / "onesatoshicoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=OneSatoshiCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OneSatoshiCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  onesatoshicoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("OneSatoshiCoin-Qt"));
