...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(pindexBest,0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(pindexBest,0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "OneSatoshiCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "OneSatoshiCoin is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        if (pindexPrev != pindexBest ||
            (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > (60*60)))
        {
            if (pindexPrev != pindexBest)
            {
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "OneSatoshiCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "OneSatoshiCoin is downloading blocks...");
